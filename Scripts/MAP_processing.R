...
MAP_Hydro <- read_csv('data/ch1_studySites_wse.csv',
                      na = "no data",
                       col_types = cols(
...
MAP_GE <- read_csv('data/ch1_studySites_ge.csv')
...
MAP_WaterDepth2 <- MAP_WaterDepth %>%
  filter(!CalendarYear == 2004)

# Lets link together the transect region (West or East) by site with the processed hydrology df
t_region <- read.csv("data/ALL_MAP_T_Region.csv")


MAP_WaterDepth2 <- MAP_WaterDepth2 %>%
  left_join(t_region, by = "Site")


MAP_WaterDepthE <- MAP_WaterDepth2 %>% filter(T_Region %in% c("M1", "M3E",
                                                              "M4E_1", "M4E_2",
                                                              "M5W", "M5E"))
MAP_WaterDepthW <- MAP_WaterDepth2 %>% filter(T_Region %in% c("M3W", "M4W"))
